<?php

use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/

Route::middleware('auth:api')->get('/user', function (Request $request) {
    return $request->user();
});
Route::post('/register', [\App\Http\Controllers\Api\V1\UserAuthController::class, 'register']);
Route::post('/login', [\App\Http\Controllers\Api\V1\UserAuthController::class, 'login']);


Route::prefix('applications')->group(function (){
    Route::post('/new', [\App\Http\Controllers\ApplicationController::class, 'createNew']);
    Route::post('/type/{application}', [\App\Http\Controllers\ApplicationController::class, 'createType']);

    Route::get('/', [\App\Http\Controllers\ApplicationController::class,'index']);

});














